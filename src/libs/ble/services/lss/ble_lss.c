...
static void on_write(ble_lss_t *p_lss, ble_evt_t *p_ble_evt)
{
    ret_code_t err_code;
        ...
        {
            tsl_int_time_t int_time = *((tsl_int_time_t *) p_evt_write->data);

            uint32_t err_code;
            err_code = sensor_ctrl_set_tsl_int_time(int_time);
        		...
        		p_lss->sensor_gain_handle.value_handle)
        {
            ...
            uint32_t err_code;
            err_code = sensor_ctrl_set_tsl_gain(gain);
...
static void ble_lss_notify(ble_lss_t *p_lss,
    ...
    hvx_params.p_data = (uint8_t*)&p_lss->p_sensor_data->ir_lux;

    sd_ble_gatts_hvx(p_lss->conn_handle, &hvx_params);

    /* Notify the pressure data */
    len = sizeof(p_lss->p_sensor_data->vis_lux);

    hvx_params.handle = p_lss->visible_data_handle.value_handle;
    hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
    hvx_params.offset = 0;
    hvx_params.p_len  = &len;
    hvx_params.p_data = (uint8_t*)&p_lss->p_sensor_data->vis_lux;
    sd_ble_gatts_hvx(p_lss->conn_handle, &hvx_params);
}
...
static void ble_lss_infrared_lux_char_init(ble_lss_t *p_lss)
{
    ...
    //Set characteristic length in number of bytes and value
    attr_char_value.max_len     = sizeof(p_lss->p_sensor_data->ir_lux);
    attr_char_value.init_len    = sizeof(p_lss->p_sensor_data->ir_lux);
    attr_char_value.p_value     = (uint8_t *) &(p_lss->p_sensor_data->ir_lux);
...
static void ble_lss_visible_lux_char_init(ble_lss_t *p_lss)
{
    ...
    //Set characteristic length in number of bytes and value
    attr_char_value.max_len     = sizeof(p_lss->p_sensor_data->vis_lux);
    attr_char_value.init_len    = sizeof(p_lss->p_sensor_data->vis_lux);
    attr_char_value.p_value     = (uint8_t *) &(p_lss->p_sensor_data->vis_lux);
